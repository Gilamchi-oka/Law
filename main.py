...
async def handle_admin(event):
    ...
    if text.startswith("/limit"):
        ...
        if bc.is_running():
            await event.respond("⚠️ Рассылка в Telegram уже идёт. Остановить: /stopbroadcast")
            return
        parts = text.split()
        limit = int(parts[1]) if len(parts) == 2 and parts[1].isdigit() else None
        bc._broadcast_task[0] = asyncio.create_task(
            bc.run_broadcast(client, REPORT_CHAT_ID, ADMIN_ID, limit)
        )

    elif text.startswith("/stopbroadcast"):
        bc.stop_broadcast()
        await event.respond("🛑 Останавливаю рассылку в Telegram...")
